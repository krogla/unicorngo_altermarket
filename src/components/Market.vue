<template>
  <b-container>
    ...
    <!--:img-src="'https://s3.unicorngo.io/unicorn/'+offer.unicornId+'-x.png'"-->
    <!--img-src="https://s3.unicorngo.io/unicorn/0-x.png"-->
    <!--v-for="(chunk, rowIdx) in offersChunks"-->
      ...
      <li class="list-inline-item" v-for="item in currentPageItems" :key="item.unicornId">
        ...
        <b-card  :sub-title="'#'+item.unicornId"
                ...
                img-top
                 ...
                 class="mb-2">
          <p class="card-text">
            <b-badge variant="primary">&nbsp;{{item.priceEth}}&nbsp;<svg class="card__ether"><use xlink:href="#ethereum"></use></svg></b-badge>
...
export default {
  ...
  computed: {
    ...
    currentPageLoadedItemsCount () {
      ...
      // return this.currentPageItems.reduce((loaded, item) => {return loaded + typeof item === 'undefined' ? 0 : 1}, 0)
      // return this.items.slice(this.currentPageFirstIndex, this.currentPageLastIndex).reduce((loaded, item) => loaded + typeof item === 'undefined' ? 0 : 1, 0)
      // return this.pagination.pages.reduce((loadedSum, page) => loadedSum + page.loaded ? 1 : 0)
    },
    pagesCount () {
  ...
  methods: {
    ...
    initItems () {
      ...
      this.br_contract.functions.marketSize().then(function (cnt) {
    ...
    loadItem (index) {
      this.br_contract.functions.market(index).then(function (unicornId) {
        this.br_contract.functions.offers(unicornId).then(function (offer) {
          this.items.splice(index, 1, {
            unicornId: unicornId.toNumber(),
            priceEth: ethers.utils.formatEther(offer.priceEth),
            priceCandy: ethers.utils.formatEther(offer.priceCandy)
          })
  ...
  created () {
    ...
    //   this.provider = new ethers.providers.Web3Provider(this.currentProvider)
    //   console.log(this.currentProvider)
    //   console.log(this.currentProvider.isConnected())
    //   this.provider.listAccounts().then(function (acc) {
    //     console.log(acc)
    //   })
    // }
    // this.provider.getBlockNumber().then(function (blockNumber) {
  ...
  mounted () {
    ...
    if (this.isWeb3) {
      ...
      //   network = this.getNetwork(web3.currentProvider.publicConfigStore.getState().networkVersion)
      // }
      // // console.log(this.currentProvider.publicConfigStore.getState().networkVersion)
      // console.log(web3.currentProvider.isConnected())
      // console.log(web3.currentProvider.net_version())

      this.provider = new ethers.providers.Web3Provider(web3.currentProvider)
      console.log(this.provider.version.getNetwork())
